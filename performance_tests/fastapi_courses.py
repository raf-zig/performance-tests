...
class CourseIn(BaseModel):
    """
    Модель для приёма входных данных (создание и обновление courses).
    """
...
class CourseOut(CourseIn):
    """
    Модель для отдачи данных о курсe, включая его ID.
    """
    id: int  # Уникальный идентификатор course
...
class CoursesStore(RootModel):
    ...
    """
    root: list[CourseOut]  # Список всех courses

    def find(self, course_id: int) -> CourseOut | None:
        """
        Находит course по ID.
    ...
    def create(self, course_in: CourseIn) -> CourseOut:
        """
        Создаёт нового course, генерируя для него следующий ID.
        """
    ...
    def update(self, course_id: int, course_in: CourseIn) -> CourseOut:
        """
        Обновляет существующего course по ID.
        """
    ...
    def delete(self, course_id: int) -> None:
        """
        Удаляет course по ID, фильтруя список.
        """
...
async def get_course(course_id: int):
    ...
    Возвращает course по ID или 404, если не найден.
    """
...
async def create_course(course: CourseIn):
    """
    POST /api/v1/courses
    Создаёт нового course и возвращает его данные с ID.
    """
...
async def update_course(course_id: int, course: CourseIn):
    ...
    Обновляет данные course по ID или возвращает 404, если не существует.
    """
...
async def delete_course(course_id: int):
    ...
    Удаляет course по ID или возвращает 404, если не существует.
    """
    if not store.find(course_id):
        raise HTTPException(
            detail=f"User with id {course_id} not found",
