...
# Шаг 2. пополняем счет для только что созданного пользователя
operations_gateway_client_response = operations_gateway_client.make_top_up_operation(
    account_id=open_debit_card_account_response.account.id,
    card_id=open_debit_card_account_response.account.cards[0].id
)
print('Make top up operation response:', operations_gateway_client_response)